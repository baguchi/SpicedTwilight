...
public class HidingGoal extends Goal {
    ...
    public boolean canUse() {
        return this.helmetCrab.getData(ModAttachments.HELMET_CRAB_HIDE) != null && this.helmetCrab.getData(ModAttachments.HELMET_CRAB_HIDE).isHide();
    }

    @Override
    public void tick() {
        super.tick();
        HelmetCrabHideAttachment attachment = this.helmetCrab.getData(ModAttachments.HELMET_CRAB_HIDE);

        if (attachment != null) {

            if (attachment.getHideCooldown() <= 0) {
                attachment.hideStop();
